(ns io.ivy.bukkit.core
  (:require [io.ivy.bukkit.bukkit :as bk]
            [io.ivy.bukkit.blocks :as blocks]
            [io.ivy.bukkit.events :as ev]
            [io.ivy.bukkit.entity :as ent]
            [io.ivy.bukkit.player :as plr]
            [io.ivy.bukkit.util :as util]
            [io.ivy.bukkit.logging :as log]
            [io.ivy.bukkit.config :as cfg]
            [io.ivy.bukkit.commands :as cmd]
            [io.ivy.bukkit.recipes :as r]
            [io.ivy.bukkit.items :as i]
            [io.ivy.bukkit.files]
...
;;   (cond
;;    @repl-handle {:msg "you tried to start a(nother) repl while one was already started"}
;;    :else (do (reset! repl-handle (start-server :host host :port port :handler cider-nrepl-handler))
